...
def train_one(task, fast, args, total_grad):
    ...
    for i in range(args.maml_innersteps):
        if i == 0:
            ...
            #print("weights:", fast_weights['ebd'])

            #XS = fast['ebd'](support, fast_weights['ebd'])  ######### not using lstmatt as embedding

            support['weights'] = fast_weights['ebd']   ######### using lstmatt as embedding
            XS = fast['ebd'](support)                
            ...
            grads = torch.autograd.grad(
                    loss,
                    itertools.chain(support['weights'].values(),
                                   fast_weights['clf'].values()),
                    create_graph=True,####over
                    allow_unused=True)

        # update fast weight
        support['weights'] = OrderedDict(
    ...
    query['weights'] = fast_weights['ebd']
    XQ = fast['ebd'](query)
