...
def tsne(x,y):
    ...
    tsne = TSNE(n_components=2, init='random',random_state=500).fit(x)
    ...
    plt.xlim((-70, 70))
    plt.ylim((-70, 70))
    plt.scatter(df2[0], df2[1], alpha=1,color='orange', label='cover')
    plt.scatter(df1[0], df1[1], alpha=0.2,color='steelblue', label='stego')

    plt.legend()
    path="./BLOCK_tn_5.jpg"        
    plt.savefig(path) 
    plt.close()  
    # result=tsne.fit_transform(x_test)s
    # fig=plot_embedding(result,y_test,'t-sne')
...
class R2D2(BASE):
    ...
    def forward(self, XS, YS, XQ, YQ):
        ...
        if self.args.mode=='test':
            tsne(XQ.cpu().detach().numpy(),YQ.cpu().detach().numpy())
